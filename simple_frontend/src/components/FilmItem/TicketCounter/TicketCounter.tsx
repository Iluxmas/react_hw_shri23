...
function TicketCounter({ id, onDelete }: TickerCOunterProps) {
  const ticketsAmount = useSelector((state) => selectProductCount(state, id));
  const dispatch = useDispatch();

  const path = usePathname();

  const handleMinus = ticketsAmount > 1 ? () => dispatch(cartActions.decrement(id)) : onDelete;
  ...
  return (
    <div className={styles.film__tickets}>
      ...
      ></button>
      {path === '/checkout' && <button className={styles.film__delete} onClick={onDelete}></button>}
    </div>
  );
}
